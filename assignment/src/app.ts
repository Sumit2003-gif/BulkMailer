...
import { getCookie } from "hono/cookie"; // 👈 Ye line add karein
// Import middleware
...
// Middleware
app.use('/*', cors({
  // '*' ki jagah apna exact frontend URL likhein
  origin: 'http://localhost:5173', 
  credentials: true, // Ye line cookies allow karne ke liye compulsory hai
...
app.use("*", async (c, next) => {
  ...
  if (path === "/") {
    ...
    if (!token) {
    return c.json({ success: false, message: "⚠️ Authentication required" }, 401);
    }
