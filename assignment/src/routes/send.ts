...
app.post("/send", async (c) => {
  try {
   ...
   console.log("📥 RECEIVED FIELDS:", Array.from(formData.keys()));
console.log("📂 FILE RECEIVED:", formData.get("excelFile") ? "YES" : "NO");
    ...
    if (missingFields.length > 0) {
      console.log("❌ Validation failed for fields:", missingFields); // Ye line add karein
      return c.json(
        {
          success: false,
